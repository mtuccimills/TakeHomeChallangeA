...
from db import get_db
from routers.pokemons.entity import UserPokemonsResponse
...
def get_service(
    ...
    return UserService(repository, pokeapi_client)
...
@router.get("/{user_id}/pokemon", response_model=UserPokemonsResponse)
...
async def delete(
    user_id: int, current_user: CurrentUser, service: UserService = Depends(get_service)
):
