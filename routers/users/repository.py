...
from models import User, Pokemon, PasswordResetToken
...
#            .where(Pokemon.user_id == id)
        ...
        has_more = skip+len(pokemons) < total
        

        return PaginatedPokemonResponse(
            pokemons=[PokemonResponse.model_validate(pokemon) for pokemon in pokemons],
            total=total,
            skip=skip,
            limit=limit,
            has_more=has_more,
    ...
    async def hash_token_exists(self, 
                             token_hash: str,
                             ) -> PasswordResetToken:
        result = await self.db.execute(
    ...
    async def hash_token_expired(self, 
                             token_hash: str,
                             ) -> PasswordResetToken:
    ...
    async def hash_token_delete(self, 
                             user_id: int,
                             ) -> PasswordResetToken:
        await self.db.execute(
            sql_delete(PasswordResetToken)
